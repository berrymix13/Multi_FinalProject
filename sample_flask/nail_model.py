...
def fit_nail(hand_raw: str, hand_mask: str, design: str) -> np.ndarray :
    ...
    raw_hand_dir = "static/adain/hand"
    raw_hand = cv2.imread(f"{raw_hand_dir}/{hand_raw}", cv2.IMREAD_UNCHANGED)
    raw_hand = bgrrgb(raw_hand)

    mask_hand_dir = 'static/adain/hand_mask'
    raw_mask = cv2.imread(f"{mask_hand_dir}/model_mask_{hand_mask}")
    ...
    nail_img = cv2.imread(f"static/image/nail_design_mask/{design_name}.jpg")
    nail_img = bgrrgb(nail_img)
    crop_nail_img = rot_crop_box3(nail_img)

    for num, dst in enumerate([i for i in crop_nail_img if i.shape[0]+i.shape[1] > 7]):
        ...
        save_path = 'static/nail_make/'
        plt.imsave(save_path + 'sample.jpg', raw_hand)
