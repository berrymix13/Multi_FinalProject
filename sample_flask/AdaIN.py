...
vgg_path = 'static/adain/vgg_normalised.pth'
decoder_path = 'static/adain/decoder.pth'
...
design_json = json.load(open("static/adain/design_gen.json", encoding="UTF-8"))

def set_design(user_pt, user_st1, user_st2, shape):
    if user_pt == '자개':
        design0 = design_json['패턴']['풀컬러'][0]
        design1 = design_json['패턴'][user_pt][random.randint(0, len(design_json['패턴'][user_pt]) - 1)]
        design2 = design0
        design3 = design_json['패턴']['마블'][random.randint(0, len(design_json['패턴']['마블']) - 1)]
        design4 = design1
    ...
    design0 = Image.open('static/adain/pattern/'+design0) 
    design1 = Image.open('static/adain/pattern/'+design1) 
    design2 = Image.open('static/adain/pattern/'+design2) 
    design3 = Image.open('static/adain/pattern/'+design3) 
    design4 = Image.open('static/adain/pattern/'+design4)

    if user_st1 == "동양화":
        path = 'static/adain/그림/동양화/'
    elif user_st1 == "만화":
        path = 'static/adain/그림/만화/'
    elif user_st1 == "서양화":
        path = 'static/adain/그림/서양화/'
    elif user_st1 == "팝아트":
        path = 'static/adain/그림/팝아트/'
     
    style_path = path+user_st2
    final_path = path.split('adain/')[1]
    
    # 이미지 생성  : 30초 정도 소요됨
    fname = 'practice_1'     
    for num in range(5):
        
        content = content_tf(eval(f'design{num}'))
        ...
        save_image(output, f'static/adain/sample/{fname}_design{num}.png')
    ...
    shape_path = 'static/adain/nail_shape/'+user_shape +'.jpg'

    # 생성된 디자인 불러오기
    gen_path = 'static/adain/sample/'
    ...
    save_path = 'static/adain/result/'  
    plt.imsave(save_path + f'{fname}.jpg', gen_shape1);

    return fname, user_shape, final_path
