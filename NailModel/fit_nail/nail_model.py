...
def fit_nail(hand_raw: str, hand_mask: str, design: str) -> np.ndarray :
    ...
    '''
    raw_hand = cv2.imread(f"./NailModel/fit_nail/{hand_raw}", cv2.IMREAD_UNCHANGED)
    raw_mask = cv2.imread(f"./NailModel/fit_nail/{hand_mask}")
    ...
    nail_img = cv2.imread(f"./NailModel/fit_nail/design/{design}")
