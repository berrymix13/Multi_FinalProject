...
def brightness(nail, hand_resize):
    nail_copy = nail.copy()
    val = (nail.mean() - hand_resize.mean())*0.5
    arr = np.full(nail.shape, (val, val, val), dtype=np.uint8)
    if (nail.mean() - hand_resize.mean()) < 0 :
        nail_copy = cv2.subtract(nail_copy, arr)
    else:
        nail_copy = cv2.subtract(nail_copy, arr)
    return nail_copy
