...
from mask_segmentation import test_dataset      # mask_segmentation 모델 불러옴
from MediaPipe import find_point, degree_finger
#################### ObjectDetection + MediaPipe Only ########################
##############################################################################
############################ 함수 정의부 #####################################
...
def add_nail(design_bt, image, crop_img, degree_lst, side_x_lst, side_y_lst, index, texture_name):
    texture = cv2.imread(f'./texture/{texture_name}.jpg')
    ...
    for i in range(len(crop_img)):
        ...
        if texture_name == 'stilleto' or texture_name == 'ballerina' :
...
# 손 이미지  불러오기
img_name = 'hand3.jpg'
image = cv2.imread(f"./hand/{img_name}")
image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

test_dataset_dir = Path('./hand')
...
son_mask = cv2.imread(f"./hand_mask/model_mask_{img_name}")
son_mask = cv2.resize(son_mask, (image.shape[1], image.shape[0]))
rot_crop_box3(son_mask)
print('3완')

# 생성된 디자인 불러오기
design_name = 'pop_check2'
design = cv2.imread(f'./ADAIN/{design_name}.jpg')
...
result = add_nail(design_bt, image, crop_img, degree_lst, side_x_lst, side_y_lst, index, texture_name)
plt.axis('off')
plt.imshow(result)
plt.imsave(f'result_1_{design_name}_{img_name}', result)
plt.show()
